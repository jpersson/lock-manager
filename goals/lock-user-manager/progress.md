...
- **E2E bug found (step 11, HAOS): no keypad events on Onesti Nimly.** The Nimly does not publish `action`/`action_user` events at all — it reports activity via *state fields* (`last_unlock_source`/`last_unlock_user`, `last_lock_source`/`last_lock_user`). Fixed in 0.1.3 with a second event style: last_* tuple changes plus lock-state transitions (which also catch repeated unlocks by the same user). Also fixed the scrubber: the Nimly publishes `last_used_pin_code` (the actual PIN used) in its state — log scrubbing now redacts any pin/password/secret/token key.
- **E2E feedback round 2 (0.1.4):** per unlock the user saw 3 activity entries — a stale `fingerprintsensor` event (state-transition fallback firing with the *previous* tuple before the real `last_*` update message arrived), the correct keypad event, and the auto-relock. Fixes: state-transition fallbacks held briefly and dropped when the real tuple change supersedes them; unlock notifications coalesced with the following relock into a single "<user> unlocked <lock>. Locked after N seconds" message (`notify_coalesce_seconds`, default 15, 0 = immediate; no HA-side notification-id trick needed). Held notifications flush on shutdown. The auto-relock (source `self` → manual) never notifies on its own.
- **E2E feedback round 3 (0.1.5):** unlocking with a code from an unmanaged slot was attributed to the managed user (Johan). Cause: the Nimly's state-change message carries the *previous* unlock's last_* tuple; the state-transition fallback fired after 1.5 s — before the real tuple update arrived seconds later — emitting the stale (previous-user) attribution and notifying. Fix: fallback hold extended to 10 s (> the attribute-report lag), superseded by the real tuple change; echo absorption for the reverse order. Unmanaged slots now show "Unknown user (slot N)" with no notification. Known residual risk: if the tuple update ever lags > 10 s, the fallback still emits best-effort attribution.
